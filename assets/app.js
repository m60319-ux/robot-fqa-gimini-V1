    ...
    if (q) {
        ...
        const html = `
            <div class="article">
                ...
                ${pickText(content.notes) ? `<div class="note"><b>Note:</b> ${esc(pickText(content.notes))}</div>` : ''}
            </div>
        `;
...
function renderList(label, obj) {
    ...
    return `<div class="section"><h3>${label}</h3><ul>${list.map(i=>`<li>${esc(i)}</li>`).join('')}</ul></div>`;
}
